import ProductGrid from "@/app/components/ProductGrid";
import { searchProductsByName } from "@/sanity/lib/products/searchProductByName";
import React from "react";

async function Search({ searchParams }: { searchParams: { query: string } }) {
  const { query } = await searchParams;

  const products = await searchProductsByName(query);
  console.log({ products });

  // console.log({ query });

  if (!products?.length) {
    return (
      <>
        <div className="flex flex-col item-center min-h-screen bg-gray-100 p-4 ">
          <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-4xl mx-auto">
            <h1 className="text-3xl semi-bold mb-6 text-center">
              No products found for: <span className="font-bold ">{query}</span>
              .
            </h1>
            <p>Try searching for with different keywords.</p>
          </div>
        </div>
      </>
    );
  }

  return (
    <div className="flex flex-col items-center justify-start  min-h-screen bg-gray-100 p-4">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-4xl ">
        <h1 className="text-3xl font-bold mb-6 text-center ">
          Search result for <span className="font-mono">: {query}</span>
        </h1>
        <ProductGrid products={products} />
      </div>
    </div>
  );
}

export default Search;
